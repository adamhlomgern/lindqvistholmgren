...
- Filer ligger i en privat Supabase-storage-bucket (`attachments`) med signerade URL:er (1 timmes TTL). `project_files` (utan synlighetsflagga) är sedan 2026-09-17 avvecklad till förmån för materialbiblioteket — se nedan.
...
- [x] 2026-09-17: `project_files` avvecklad helt. Projektets "Filer"-sektion (`ProjectFilesSection`) läser/skriver nu `material_items` (taggat med `project_id`, `folder_id` null) istället för en egen tabell — samma fil är därmed synlig både på projektsidan och i kundens materialbibliotek, utan dubblettuppladdning. Uppladdning går genom `/api/admin/material-upload` (tar nu även en valfri `projectId`). Migreringsskript för befintliga `project_files`-rader: `docs/migrations/2026-09-17-merge-project-files-into-material-items.sql` (måste köras manuellt i Supabase SQL-editorn — den här miljön har ingen databasåtkomst). Kvar: projektfiler visas ännu utan mappstruktur (`Internal arbetsmaterial`/`Kundunderlag` etc från auditen är inte byggt), och kundens materialflik har inget projektfilter ännu.
...
- [x] 2026-09-24: `project_approvals` fick en `kind`-kolumn (`feedback` | `approval`) — auditens poäng att "en logotypriktning är inte samma sak som att godkänna slutleveransen". Samma status-maskin (pending/approved/changes_requested) för båda, bara begärnings-dialogens val och kundens knapptexter skiljer sig åt. Migreringsskript: `docs/migrations/2026-09-24-add-approval-kind.sql` (måste köras manuellt i Supabase, ingen databasåtkomst i den här miljön).
...
- [x] Lägg till adminförhandsgranskning av kundvyn. Byggt för Översikt, Projekt (inkl. enskild projektsida och godkännande), Material och Meddelanden (`kundvy`, `kundvy/projekt`, `kundvy/projekt/[projectId]`, `kundvy/projekt/[projectId]/godkannande/[approvalId]`, `kundvy/material`, `kundvy/meddelanden`) — samma komponenter som kundens riktiga sidor, admin-hämtad data.
