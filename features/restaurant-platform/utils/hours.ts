import type { OpeningHours, Restaurant } from "@/features/restaurant-platform/types";
...
export function todaysHours(restaurant: Restaurant, now: Date = new Date()): OpeningHours | undefined {
  return hoursForDay(restaurant, now.getDay());
}

export function isOpenNow(restaurant: Restaurant, now: Date = new Date()): boolean {
  const today = todaysHours(restaurant, now);
