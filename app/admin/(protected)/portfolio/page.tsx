  ...
  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-bone">Portfolio</h1>
          <p className="mt-1 text-sm text-stone">{projects.length} case i portföljen.</p>
        </div>
        <Link
          href="/admin/portfolio/ny"
      ...
      {projects.length === 0 ? (
        <div className="mt-8 flex flex-col items-center gap-3 rounded-2xl border border-dashed border-bone/15 px-6 py-16 text-center">
          <GalleryHorizontalEnd size={24} strokeWidth={2} className="text-stone" />
          <p className="text-sm text-stone">Inga case ännu.</p>
          <Link
            href="/admin/portfolio/ny"
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((project) => {
            ...
            return (
              <Link key={project.slug} href={`/admin/portfolio/${project.slug}`} className="group block">
