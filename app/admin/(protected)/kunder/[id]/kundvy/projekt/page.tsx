...
export default async function CustomerKundvyProjectsTab({ params }: Props) {
  const { id } = await params;
  const projects = await getClientProjectsByCustomerId(id);
    ...
    <div>
      ...
      <div className="mt-6 flex flex-col gap-3">
        {active.length === 0 ? (
          ...
          active.map((project) => <ProjectListRow key={project.id} project={project} hrefBase={hrefBase} />)
        )}
      </div>
        ...
        <div className="mt-8">
          ...
          <div className="mt-3 flex flex-col gap-3">
            {finished.map((project) => (
              <ProjectListRow key={project.id} project={project} hrefBase={hrefBase} />
            ))}
          </div>
        </div>
      )}
