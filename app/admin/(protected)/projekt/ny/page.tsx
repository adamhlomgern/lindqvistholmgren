...
export default async function NewClientProjectPage({ searchParams }: Props) {
  const [{ customer }, customers, billingEntities] = await Promise.all([
    searchParams,
    getCustomers(),
    getBillingEntities(),
  ]);
  ...
  return (
    <div>
      ...
      <Card className="mt-8 max-w-3xl">
        <ClientProjectForm customers={customers} billingEntities={billingEntities} initialCustomerId={customer} />
      </Card>
    </div>
  );
}
