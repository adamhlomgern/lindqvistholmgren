...
import { getRecentEmailsCount } from "@/lib/data/emails";
import { resolveAdminDisplayName } from "@/lib/format";
import { RECENT_INQUIRY_WINDOW_DAYS, RECENT_EMAIL_WINDOW_DAYS } from "@/lib/constants";
...
const getCachedSidebarCounts = unstable_cache(
  async () => {
    const [newInquiriesCount, invoiceStats, activeProjectsCount, waitingChatCount, newEmailsCount] = await Promise.all([
      getRecentInquiriesCount(RECENT_INQUIRY_WINDOW_DAYS),
      getInvoiceStats(),
      getActiveClientProjectsCount(),
      getWaitingChatThreadCount(),
      getRecentEmailsCount(RECENT_EMAIL_WINDOW_DAYS),
    ]);
    return {
      newInquiriesCount,
  ...
  return (
    <div className="flex flex-col bg-forest text-bone md:h-screen md:flex-row">
      <AdminSidebar
        ...
        newEmailsCount={counts.newEmailsCount}
      />
